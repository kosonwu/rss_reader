...
function StatusBadge({ status }: { status: NerLog["status"] }) {
  if (status === "success") {
    return (
...
export default function FetchNerLogsClient({
  logs,
  feedsCount,
  subscriptionsCount,
  keywordsCount,
        ...
        <div className="max-w-7xl mx-auto flex items-end justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 mb-2">
              <ScanTextIcon className="size-4 text-amber-400" />
      ...
      <div className="max-w-7xl mx-auto px-6 py-8 lg:px-10">
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-32 text-muted-foreground gap-4">
            <ScanTextIcon className="size-14 opacity-10" />
            <p className="font-mono text-sm">No NER logs in this date range.</p>
          </div>
        ) : (
          <>
